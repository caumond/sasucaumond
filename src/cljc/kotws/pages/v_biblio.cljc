...
(def dic
  {:desc-title {:en "Sum-up", :fr "Synthèse"},
   ...
   :clojure-details
     {:fr
        "Ce livre a longtemps été mon livre de chevet, après avoir appris par ailleurs les bases de clojure, il permet de bien en appréhender les détails, l'esprit du language. La première lecture ne m'a pas suffit, il faut y revenir régulièrement pour bien intégrer les éléments.",
      :en
  ...
  (let [tr (partial klang/tr dic l)
    ...
    [:<> [:h1 (tr :biblio-title)] [:p (tr :biblio-intro)]
     (let [{:keys [title sub-title img details detailed-desc]}
       ...
       [:div
        [:div.w3-center
         [kvselector/v-selector (count books) selected
          (partial opts-go-to (count books))
          (partial opts-go-rel (count books))]] [:h2.text title]
        (when-not (nil? sub-title) [:h3.text sub-title])
        ...
        [:div.w3-center
         [kvselector/v-selector (count books) selected
          (partial opts-go-to (count books))
          (partial opts-go-rel (count books))]] [:hr]])]))
