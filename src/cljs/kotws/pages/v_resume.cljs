...
(defn tr
  [k]
  (-> {:title {:fr "Curriculum vitae", :en "Resume"},
       :public-reference {:fr "Référence en ligne", :en "Online reference"},
       :download {:fr "Télécharger ce document", :en "Download that document"},
       :what-is-it {:fr "Qu'est-ce?", :en "What is it?"}}
      (ml/tr* k)))
...
(def resume-timeline
  [{:icon "fa-wrench",
    ...
    :ml {:en {:title "Product owner of an in-house PLM", :where "Michelin"},
...
(defn resume-panel
  []
  [:<>
   [:a {:href "docs/caumond-resume.pdf", :target "blank"}
    [:i.fa.fa-download.w3-margin.w3-right]] [:h1 (tr :title)] [:p (tr :intro)]
   [:p (tr :intro2)] [:div]])
...
;;         [:div [:div [:i.fa {:class (:icon position)}]] [:div (:date
;;         position)]
;;          [:div [:div (tr position :title)]
;;           (let [where-link (:where-link position)]
;;             (if (nil? where-link)
;;               [:p.where (tr position :where)]
;;               [:a {:href where-link, :target "blank"}
;;                [:p.where (tr position :where)]])) [:hr.round]
;;           [:ul
;;            (doall
;;              (for [role-idx (range (count (:roles position)))
;;                    :let [role (nth (:roles position) role-idx)]]
;;                ^{:key (str "role-" position-idx "-" role-idx)}
;;                [:li.role [:i.fa.role-icon {:class (:icon role)}]
;;                 [:div.w3-rest
;;                  (doall
;;                    (for [item-idx (range (count (:items role)))
;;                          :let [item (nth (:items role) item-idx)]]
;;                      (let [selected-items (rf/subscribe
;;                      [::subs/selected-items])
;;                            text-item (tr item :text)
;;                            detailed-shown? (contains? @selected-items
;;                                                       [position-idx role-idx
;;                                                        item-idx])
;;                            details-text (tr item item :details)]
;;                        ^{:key
;;                            (str "item-" position-idx "-" role-idx "-"
;;                            item-idx)}
;;                        [:div.item
;;                         {:on-click #(rf/dispatch [::events/selected-items
;;                                                   [position-idx role-idx
;;                                                    item-idx]]),
;;                          :style {:display "block"}}
;;                         [:a
...
;;                          [:i.arrow.fa.fa-angle-right
;;                           {:class (when detailed-shown? "w3-hide")}]
;;                           text-item]
...
;;                           [:a {:href (:public-reference item), :target
;;                           "blank"}
;;                            [:div.tooltipable [:i.fa.fa-globe]
;;                             [:span.tooltip (tr :public-reference)]]])
;;                         (when (:download item)
;;                           [:a {:href (:download item), :target "blank"}
;;                            [:div.tooltipable [:i.fa.fa-download]
;;                             [:span.tooltip (tr :download)]]])
;;                         [:div.detailed-item
;;                          {:class (when-not detailed-shown? "w3-hide")}
;;                          (when detailed-shown? details-text)]])))]]))]]]))
;;
(def resume-timeline-to-delete
  [{:icon "fa-wrench",
    :date "2021",
    :where-link "https://www.michelin.fr/",
    :ml {:en {:title "Product owner of an in-house PLM", :where "Michelin"},
      ...
      [{:icon "fa-industry",
        :items
          [{:what-is-it
           ...
           {:ml
              {:en
                 {:text "Creation, selling and materialization the vision",
                  :details
               ...
               :fr
                 {:text "Création, promotion et mise en oeuvre de la vision",
                  :details
       ...
       {:icon "fa-code",
        :items
          [{:what-is-it "https://fr.wikipedia.org/wiki/Clojure",
            :ml
              {:en
                 {:text "Clojure prototyping",
                  :details
                    "Development in clojure. Data mapping in huge maps (specter), describe a model with data samples and schema validation with malli."},
               :fr
                 {:text "Prototypage en clojure",
                  :details
                    "Développement en clojure. Mapping de données dans de grandes `map` (specter), description de modèle en illustrant par des données et en validant ces schémas en malli."}}}
           {:ml
              {:en
                 {:text "Application design ",
                  :details
                    "Design some complex concepts around release management and change orders."},
               :fr
                 {:text "Conception de l'application",
                  :details
                    "Conception de concepts complexes autour de la gestion de configuration: la gestion des versions et les ordres de changements."}}}]}]}
   {:icon "fa-ruler-combined",
    :date "2019",
    :where-link "https://www.michelin.fr/",
    :ml {:en {:title "Supply Chain Entreprise Architect", :where "Michelin"},
    ...
    :roles
      [{:icon "fa-industry",
        :items
          [{:ml
              {:en
                 {:text "C-Level entreprise studies",
                  :details
               ...
               :fr
                 {:text "Etudes `Entreprise` niveau `executives`",
                  :details
                    "Quelques études pour illustrer cette activité: L'intégration de la sous traitance dans un processus groupe, avec la prise en compte de la stratégie groupe, de la fiscalité, des douanes et logistiques. Etude sur la suite du programme de transformation OPE sur les usines de produits finis."}}}
           {:ml
              {:en
                 {:text "Software package purchasing",
                  :details
                    "I was in charge of supply chain software package purchase: a first project on an automative ERP for integration with automotive customers and their specificities. A second project to equip a subsidiary with an integrated supply chain tool."},
               :fr
                 {:text "Achat de progiciels entreprises",
                  :details
                    "J'ai eu en charge l'achat des progiciels de mon domaine supply chain: un premier projet sur un ERP spécialisé dans l'automobile pour intégration avec les clients automobiles et leurs spécificités. Un second projet pour outiller une filiale avec un outil de supply chain intégré."}}}
           {:public-reference
              "https://www.usinenouvelle.com/article/michelin-veut-reduire-ses-frais-generaux-de-500-millions-d-euros-d-ici-2020.N396442",
            :ml
              {:en
                 {:text "Framework studies for a major transformation program",
                  :details
               ...
               :fr
                 {:text "Etudes cadres d'un programme de transformation majeur",
                  :details
           ...
           {:public-reference
            ...
            :ml
              {:en
                 {:text "Sales and Operations framework and sourcing with JDA",
                  :details
                    "Finalize purchasing of software package with JDA, frame the project"},
               :fr
                 {:text "Cadrage et achat d'un progiciel S&OP avec JDA",
                  :details
                    "Finalisation de l'achat du progiciel S&OP avec JDA, cadrage du projet."}}}
           {:ml
              {:en
                 {:text "First attempt of group A.I. software purchasing",
                  :details
               ...
               :fr
                 {:text
                    "Première tentative d'achat d'un progiciel d'IA au niveau du groupe",
                  :details
       ...
       {:icon "fa-code",
        :items
          [{:ml
              {:en
                 {:text "Kownledge of main Supply Chain software package",
                  :details
               ...
               :fr
                 {:text "Connaissance des principaux progiciels Supply Chain",
                  :details
            ...
            :ml
              {:en
                 {:text "Enterprise Architecture - TOGAF",
                  :details
                    "Entreprise Architecture deliverables as described in TOGAF (cf. certification). 200+ software in my perimeters and some architects in my network."},
               :fr
                 {:text "Pratique de l'architecture entreprise",
                  :details
                    "Livraison des livrables conformément au processus Architecture Entreprise tel que décrit par TOGAF (cf. certification). 200 logiciels dans mon périmètre et quelques architectes dans mon réseau."}}}
           {:public-reference
              "https://videos.confluent.io/watch/Uny8xop6vWZut5Hb5CaNpA?",
            :ml
              {:en
                 {:text
                    "Launch of an event streaming app - the so called booster stock",
                  :details
          ...
          [{:ml
              {:en
                 {:text "Architecture business",
                  :details
                    "At the beginning of OPE program, the 60 semi finished production sites are locally managed, with a high diversity of process and maturity. My contribution is the design of supply chain process, build planning model in a PLM, integrate planning with and ERP and a WMS (Warehouse Management System)."},
               :fr
                 {:text "Architecture business",
                  :details
                    "Au démarrage du programme OPE, les 60 sites de production de semis-finis sont gérés complétement localement. Ma contribution consiste à concevoir les processus supply chain, construire le modèle de planification dans un PLM, intégrer la planification avec l'ERP et l'outil de gestion de magasin (WMS)."}}}
           {:ml
              {:en
                 {:text "Production planning",
                  :details
                    "Expert (solution consultant) of a master planning application for finish products plant - deployment on asian and south american plants."},
               :fr
                 {:text "Expert d'une solution de planning de production",
                  :details
                    "Expert (solution consultant) d'une application de master planning des usines produits finis - déploiement sur des usines d'asie et d'amérique du sud."}}}
           {:ml
              {:en
                 {:text "Certification for BASICS - APICS",
                  :details
                    "I was certified with MGCM with a BASICS level of APICS. Supply chain oriented, dealing with methods and tools, this training gives a comprehensive panorama of supply chain."},
               :fr
                 {:text "Certification BASICS à l'APICS",
                  :details
                    "J'ai été certifié par MGCM pour le niveau BASICS de l'APICS. Orientée Supply Chain, outils et méthodes, cette formation fournit un panorama complet de la supply chain."}},
          ...
          [{:ml
              {:en
                 {:text "Solution Architecture",
                  :details
                    "Solution architect of two planning solution - architecture folder."},
               :fr
                 {:text "Architecte solution",
                  :details
                    "Architecte solution sur deux solutions de planification - dossier d'architecture."}}}]}]}
          ...
          [{:ml
              {:en
                 {:text "Distribution planning - DRP - change management",
                  :details
                    "Design of distribution network, deployment of a DRP on 3 product lines (European truck tires, Agriculture, Africa India Middle east)"},
               :fr
                 {:text
                    "Planning de distribution - DRP - gestion du changement",
                  :details
                    "Conception de réseaux de distribution, de déploiement d'un DRP sur 3 lignes de produits (Poids Lourd Europe, Agro, Afrique Inde Moyen Orient)"}}}]}
       {:icon "fa-infinity",
        :items
          [{:ml
              {:en
                 {:text "Distribution network optimization",
                  :details
                    "Solving thousands of minimum flow algorithms (one per article reference) thanks to busaker and gowen algorithm. Design, development and integration of the solution."},
               :fr
                 {:text "Optimisation du réseau de distribution",
                  :details
                    "Résolution de milliers (un par référence article) de problème de flots minimum, grâce à l'algorithme de busaker et gowen (coût de transport minimum respectant les contraintes capacitaires)."}}}
           {:ml
              {:en
               ...
               :fr
                 {:text "Formateur en Supply Chain",
                  :details
                    "Vacataire en Master I et Master II en Logistique et Supply Chain."}}}]}
       {:icon "fa-code",
        :items
          [{:ml
              {:en
                 {:text "PLSQL",
                  :details
                    "Integration in PLSQL of optimization tool (Thousand of product references calculation)."},
               :fr
                 {:text "PLSQL",
                  :details
                    "Conception, développement et intégration de la solution d'optimisation du réseau de distribution."}}}
           {:ml
              {:en
                 {:text "C++ Tactical planning optimization tool",
                  :details
                    "Needs analysis - design - development of an optimization tool for tactical planning (make or buy)."},
               :fr
                 {:text "Outil tactique d'optimisation en C++",
                  :details
                    "Analyse du besoin - conception - développement d'un outil d'optimisation des plans tactiques (make or buy)."}}}]}]}
   {:icon "fa-shopping-cart",
    :date "2006",
    :where-link
      "https://web.archive.org/web/20060614232036/http:/www.bijouteriecaumond.fr/",
    :ml {:en {:title "E-commerce website", :where "Family jewelry"},
         :fr {:title "Site e-commerce", :where "Bijouterie familiale"}},
    :roles
      [{:icon "fa-industry",
        :items
          [{:ml
              {:en {:text "Practical order to cash knowledge",
                    :details "Build and practise of an order to cash process."},
               :fr
                 {:text "Connaissance pratique d'un order to cash",
                  :details
                    "Construction et mise en pratique d'un processus order to cash."}}}]}
       {:icon "fa-code",
        :items
          [{:public-reference
              "https://web.archive.org/web/20060614232036/http:/www.bijouteriecaumond.fr/",
            :ml
              {:en
                 {:text "Php5 - mysql paradox db - Borland C++",
                  :details
                    "The website was operational and worked during roughly three years. The website objective was to create extra activity for the family jewerly. A backend in Borland C++ was developped to manage and upload product catalog. The website by itself contains some e-payment and CMS to delegate some site update."},
               :fr
                 {:text "Php5 - mysql paradox db - Borland C++",
                  :details
    ...
    :roles
      [{:icon "fa-industry",
        :items
          [{:ml
              {:en
                 {:text "Industrial Proof Of Concepts",
                  :details
       ...
       {:icon "fa-code",
        :items
          [{:ml
              {:en
                 {:text "UI and backend in Borland C++",
                  :details
                    "Most of that poc are assorted with user interfaces for data preparation, result visualization, setup and optimisation monitoring."},
               :fr
                 {:text "Interfaces utilisateurs et backend en Borland C++",
                  :details
                    "Ces démonstrateurs sont pour la plupart agrémentés d'interfaces utilisateurs pour la préparation des données, la visualisation des résultats, le paramétrage et pour le suivi de l'optimisation."}}}
           {:ml
              {:en
                 {:text "C++ Framework for iterative optimization",
                  :details
                    "The chapter 5 of my thesis offers a framework for iterative optimization."},
               :fr
                 {:text "Cadriciel pour l'optimisation itérative",
                  :details
                    "Le chapitre 5 de ma thèse propose un framework pour l'optimisation itérative."}}}]}
       {:icon "fa-infinity",
        :items
          [{:download "https://tel.archives-ouvertes.fr/tel-00713587/document",
            :ml
              {:en
                 {:text "Phd thesis",
                  :details
           ...
           {:public-reference
            ...
            :ml
              {:en
                 {:text "Articles",
                  :details
                    "Various articles are presented in conferences and journals."},
               :fr
                 {:text "Articles",
                  :details
                    "Plusieurs articles issus de conférences et de journaux."}}}
           {:public-reference
           ...
           {:ml
              {:en
                 {:text "Iterative methods",
                  :details
                    "Iterative methods, integer linear programming, graph optimization"},
               :fr
                 {:text "Méthodes itératives",
                  :details
                    "Méthodes itératives, programmation linéaire en nombres entierss, optimisation de graphe"}}}]}]}
   {:icon "fa-school",
    :where-link "https://www.isima.fr/",
    :date "1998",
    :ml {:en {:title "Computer science Engineering degree", :where "ISIMA"},
         :fr {:title "Ecole d'ingénieurs en informatique", :where "ISIMA"}},
    :roles
      [{:icon "fa-industry",
        :items
          [{:ml
              {:en
                 {:text
                    "Simulation, modelling and optimization of industrial systems",
                  :details
               ...
               :fr
                 {:text
                    "Simulation, modélisation et optimisation des systèmes industriels",
                  :details
   ...
   {:icon "fa-user-alt",
    :ml {:en {:title "Coding for fun", :where "@home"},
         :fr {:title "Développement pour le plaisir", :where "@maison"}},
    :roles
      [{:icon "fa-code",
        :items
          [{:what-is-it "https://en.wikipedia.org/wiki/Demoscene",
            :ml
              {:en
                 {:text "Demo making and games",
                  :details
                    "I discovered development with game creation. They could be in text mode at that time (redefining fonts to define sprites), or in graphical mode. But I spent a lot of time to develop demos: vectoballs, rasters, overscans, wireframe 3D. I had to rediscover how to draw lines..."},
               :fr
                 {:text "Démos et jeux",
                  :details
